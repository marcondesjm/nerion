...
function hasMojibake(text) {
  return /[ÃÂ�]|â[€œ€€˜€™€“€”]/.test(text);
}

function assertCaptionReady(caption) {
  if (hasMojibake(caption)) {
    throw new Error('Caption appears to contain mojibake/encoding errors. Fix PT-BR accents before publishing.');
  }
}
...
async function main() {
  const { images, caption: captionArg, captionFile, dryRun } = parseArgs(process.argv);
  const caption = captionFile ? readFileSync(resolve(captionFile), 'utf8').trim() : captionArg;
